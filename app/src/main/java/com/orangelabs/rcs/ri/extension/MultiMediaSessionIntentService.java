 ...
 * Purpose is to retrieve the contactId from the service to build the
 * notification.
 * 
 * @author YPLO6403
 */
public class MultiMediaSessionIntentService extends IntentService {
	...
	String version = MeetAsapSplashScreen.MEET_ASAP_VERSION;
	...
	private static final String LOGTAG = LogUtils
			.getTag(MultiMediaSessionIntentService.class.getSimpleName());
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		/*
		 * We want this service to stop running if forced stop so return not
		 * sticky.
		 */
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (MultimediaMessagingSessionIntent.ACTION_NEW_INVITATION
				.equals(action)) {
			mMultimediaMessagingSession = true;
		} else {
			if (!MultimediaStreamingSessionIntent.ACTION_NEW_INVITATION
					.equals(action)) {
		...
		/*
		 * Since there is no provider associated to multimedia sessions, we must
		 * connect to the API
		 */
		...
		Log.d("myerror", "called version: " + version);
	...
	private void initiateSession(Intent intent) {
		try {
			if (mMultimediaMessagingSession) {
				MultimediaMessagingSession mms = mCnxManager
						.getMultimediaSessionApi().getMessagingSession(
								mSessionId);
				if (mms != null) {
					addSessionInvitationNotification(intent,
							mms.getRemoteContact());
				} else {
					if (LogUtils.isActive) {
						Log.w(LOGTAG, "Cannot get messaging session for ID "
								.concat(mSessionId));
					}
				}
			} else {
				MultimediaStreamingSession mss = mCnxManager
						.getMultimediaSessionApi().getStreamingSession(
								mSessionId);
				if (mss != null) {
					addSessionInvitationNotification(intent,
							mss.getRemoteContact());
				} else {
					if (LogUtils.isActive) {
						Log.w(LOGTAG, "Cannot get streaming session for ID "
								.concat(mSessionId));
					}
				}
			}
	...
	private void addSessionInvitationNotification(Intent intent,
			ContactId contact) {
		/* Create pending intent */
		Intent invitation = new Intent(intent);
		String title = "title";
		if (mMultimediaMessagingSession) {
			Log.d("meetAsapError", "Multimedia session - version: " + version);

			if (version == "A") {
				invitation.setClass(this, MessagingSessionView.class);
				invitation.putExtra(MeetAsapContactsA.EXTRA_CONTACT,
						(Parcelable) contact);
				title = "Meet ASAP 1.0";
			} else if (version == "B") {
				invitation.setClass(this, MeetAsapPreNavigationB.class);
				invitation.putExtra(MeetAsapContactsB.EXTRA_CONTACT,
						(Parcelable) contact);
				title = "Meet ASAP 1.1";
			} else if (version == "C") {
				invitation.setClass(this, MeetAsapOptionsC.class);
		...
		/*
		 * If the PendingIntent has the same operation, action, data,
		 * categories, components, and flags it will be replaced. Invitation
		 * should be notified individually so we use a random generator to
		 * provide a unique request code and reuse it for the notification.
		 */
		int uniqueId = Utils.getUniqueIdForPendingIntent();
		PendingIntent contentIntent = PendingIntent.getActivity(this, uniqueId,
				invitation, PendingIntent.FLAG_ONE_SHOT);

		String displayName = RcsDisplayName.getInstance(this).getDisplayName(
				contact);
		...
		notif.setSound(RingtoneManager
